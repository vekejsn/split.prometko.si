window.onload = async function () {
    let data = await fetch('https://api.split.prometko.si/vehicles').then(res => res.json()).then(res => res.data);
    let busData = await fetch('./buses.json').then(res => res.json());
    let table = document.getElementById('table');
    // sort the data by data[i].vehicle.name
    await data.sort((a, b) => {
        if (a.garageNumber.replace(/[^\d-]/g, "") < b.garageNumber.replace(/[^\d-]/g, "")) return -1;
        if (a.garageNumber.replace(/[^\d-]/g, "") > b.garageNumber.replace(/[^\d-]/g, "")) return 1;
        return 0;
    });
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let bus = await busData.find(bus => parseInt(bus.id) == parseInt(data[i].garageNumber.replace(/[^\d-]/g, "")));
        if (bus == undefined) {
            bus = {
                id: data[i].garageNumber.replace(/[^\d-]/g, ""),
                name: data[i].name,
                type: "Nepoznato",
                model: "Nepoznato",
                plates: "Nepoznato",
        ...
        cardTitle.innerHTML = `<b>${data[i].garageNumber.replace(/[^\d-]/g, "")}</b>` + ` <small>${bus.plates}</small>`;
        ...
        cardText.innerHTML += `<b>Status: </b>${new Date() - new Date(data[i].timestamp) < 300 * 1000 ? "Aktivan" : "<em>Neaktivan</em>"}<br>`;
        cardText.innerHTML += `<hr class="vehicleinfo">`;
        if (data[i].departureTime) {
            cardText.innerHTML += `<p class="vehicleinfo"><b>Linija: </b><b class="route_name_number">${data[i].routeShortName}</b></p>`;
            cardText.innerHTML += `<hr class="vehicleinfo">`;
        } else {
            cardText.innerHTML += `<p class="vehicleinfo"><b>Zadnja linija: </b><b class="route_name_number">${data[i].routeShortName}</b></p>`;
            cardText.innerHTML += `<hr class="vehicleinfo">`;
        }
